...
def dist_adapt_regplot(
        df, yvar, xvar='dist', xerrvar=None, yerrvar=None,
    ...
    if annotate:
        ...
        for i, a in df.iterrows():
            ...
            texts.append(ax.text(a[xvar], a[yvar], lab, color="#696969", fontsize=annotation_fontsize))
    ...
    if show_correlation:
        plt.rc('legend',
               fontsize=legend_fontsize)  # ,borderpad=0.2,handletextpad=0, handlelength=0) # using a size in points
        ax.legend([ax.get_lines()[0]], ax.get_legend_handles_labels()[-1], handlelength=1.0,
                  loc=legend_loc)  # , handletextpad=0.0)
    ...
    if not no_title:
        if other_title_kind is None:
            ax.set_title(f'Distance vs Transfer Performance' + (f'({title_type})' if title_type else '')
                         + (': {}'.format(title) if title else ''),
                         fontsize=title_fontsize)
        else:
            ax.set_title(f'{other_title_kind}' + (f'({title_type})' if title_type else '')
                         + (': {}'.format(title) if title else ''),
                         fontsize=title_fontsize)
    if lim:
        ax.set(ylim=lim)
