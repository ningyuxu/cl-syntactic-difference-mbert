...
def train_model(src_lang, gpu_id=0):
    task = enum.Task().train
    hparams = get_args(task)
    exp = Experiment(hparams)
    exp.model = enum.Model().parsing
    ...
    exp.tgt_langs = [ids[langs.index(lang)] for lang in [src_lang]]
    exp.data_labels = UD_HEAD_LABELS
    checkpoint = exp.find_checkpoint()
    if os.path.isfile(checkpoint):
        print(f"Find Trained Model {checkpoint}")
    else:
        exp.init_model_home()
        exp.run([gpu_id])
...
def get_all_dists(dist_kind, src_lang, langoi="Persian", gpu_id=5, layer=12, verbose=True):
    train_model(src_lang, gpu_id=gpu_id)
    src_dir = analyse_and_path(src_lang, langoi, gpu_id=gpu_id)
    ids = [k for k, _ in ISO638_TO_LANG.items()]
    langs = [v for _, v in ISO638_TO_LANG.items()]
    dist_dict = dict()
    for _, tgt_lang in ISO638_TO_LANG.items():
        if tgt_lang == langoi or \
                tgt_lang in ["Catalan", "Czech", "Hungarian", "Urdu", "Tamil", "Chinese"]:
            continue
        dist_dict = dict()
        res_file = os.path.join(REPORT_PATH, f"{dist_kind}_{ids[langs.index(src_lang)]}.json")
        lang_pair = [ids[langs.index(lang)] for lang in [langoi, tgt_lang]]
        if os.path.exists(res_file):
            with open(res_file, mode='r', encoding="utf-8") as f:
                ...
                if f"{lang_pair[0]}_to_{lang_pair[1]}" in lang_pairs_done:
        ...
        if dist_kind == "a-distance":
            src_path_un = os.path.join(src_dir, f"outputs-{ids[langs.index(langoi)]}-1.hdf5")
            tgt_path_un = os.path.join(tgt_dir, f"outputs-{ids[langs.index(tgt_lang)]}-1.hdf5")
            dataset_un = a_distance.HDF5Dataset(filepaths=[src_path_un, tgt_path_un], langs=lang_pair, layer_index=12)
            src_path = os.path.join(src_dir, f"outputs-{ids[langs.index(langoi)]}-0.hdf5")
            tgt_path = os.path.join(tgt_dir, f"outputs-{ids[langs.index(tgt_lang)]}-0.hdf5")
            dataset = a_distance.HDF5Dataset(filepaths=[src_path, tgt_path], langs=lang_pair, layer_index=12)
            ...
            for loi in otdd.LOIs:
                src_path = os.path.join(src_dir, f"outputs-{ids[langs.index(langoi)]}-0.hdf5")
                tgt_path = os.path.join(tgt_dir, f"outputs-{ids[langs.index(tgt_lang)]}-0.hdf5")
                d, label_dist, src_classes, tgt_classes = otdd.compute_otdd(filepaths=[src_path, tgt_path],
                                                                            langs=lang_pair, loi=loi,
                                                                            layer_index=layer,
                                                                            maxsize=2000, maxsamples=1500,
        ...
        write_dist(dist_dict, src_lang_iso=ids[langs.index(src_lang)],
                   dist_kind=dist_kind, layer=layer)
